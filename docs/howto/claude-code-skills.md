...
Every skill needs a `SKILL.md` file with two parts: YAML frontmatter (between `---` markers) and markdown content with instructions Claude follows when the skill is invoked. The `name` field becomes the `/slash-command`, and the `description` helps Claude decide when to load it automatically.
...
**Task content** gives Claude step-by-step instructions for a specific action, like deployments, commits, or code generation. These are often actions you want to invoke directly with `/skill-name`. Add `disable-model-invocation: true` to prevent Claude from triggering it automatically.
...
All fields are optional except `description` is recommended so Claude knows when to use the skill.
...
Example — deploy skill only you can trigger:
...
> **Warning:** `context: fork` only makes sense for skills with explicit instructions. If your skill contains guidelines like "use these API conventions" without a task, the subagent receives the guidelines but no actionable prompt.
...
**Example: codebase visualizer skill setup:**
...
The bundled Python script does the heavy lifting (scanning directories, generating HTML) while Claude handles orchestration. This pattern works for any visual output: dependency graphs, test coverage reports, API documentation, or database schema visualizations.
